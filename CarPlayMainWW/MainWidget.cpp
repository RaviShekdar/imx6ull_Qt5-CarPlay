...
QString musicAppPath = "./MusicPlayerWW.exe";
int preNum = 0;
...
void MainWidget::init_ui()
{
    ...
    timerDT->start(200);
    connect(timerDT, &QTimer::timeout, [&]()
    {
        QDate currentDate = QDate::currentDate();  // 获取当前日期
        ui->dateLabel->setText(QStringLiteral("%1-%2-%3").arg(currentDate.year()).arg(currentDate.month()).arg(currentDate.day()));
